...
def save_response(fragment_id, human_machine_score, legit_fake_score, time_to_answer):
    """
...
if st.session_state.form_submitted:
    ...
    if st.session_state.form_submitted:
        ...
        with st.form(key=f"news_fragment_{current_fragment['FragmentID']}"):
            ...
            human_machine_score = st.select_slider("Human or Machine Generated?", options=list(human_machine_score_options.keys()), format_func=lambda x: human_machine_score_options[x], key=f"hm_score_{current_fragment['FragmentID']}")
            ...
            legit_fake_score = st.select_slider("Legit or Fake News?", options=list(legit_fake_score_options.keys()), format_func=lambda x: legit_fake_score_options[x], key=f"lf_score_{current_fragment['FragmentID']}")
            ...
            if submitted:
                with st.spinner('Wait for it...'):
                    ...
                    save_response(current_fragment['FragmentID'], human_machine_score, legit_fake_score, time_to_answer)
